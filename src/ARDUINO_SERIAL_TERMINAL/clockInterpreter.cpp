...
void ClockInterpreter::run(File &file){
    SD.begin(SD_CS);
...
void handle_SYS_function(uint8_t buffer[4], Memory &memory, File &file) {
  switch(buffer[1]){
    case PRT:
        SYS_print(buffer, memory); // Imprime el valor del banco como ASCII
      break;
    case INP:
        SYS_input(buffer, memory); // Ingresa el valor en ASCII en el banco
     break;
    case VAL:
        SYS_print(buffer, memory); // Imprime el valor del banco
      break;
    case FPO:
...
void SYS_print(uint8_t buffer[4], Memory &memory) {
  switch(buffer[3]){ // 0: SYS 1: PRT/VAL 2: <value> 3: <bank>
    case A:
        if(buffer[1] == PRT && isPrintable(memory.bank_A[buffer[2]])) {Serial.print((char)memory.bank_A[buffer[2]]); }
        else if(buffer[1] == VAL) {Serial.print(memory.bank_A[buffer[2]]); } 
      break;
    case B:
        if(buffer[1] == PRT && isPrintable(memory.bank_B[buffer[2]])){ Serial.print((char)memory.bank_B[buffer[2]]); }
        else if(buffer[1] == VAL){ Serial.print(memory.bank_B[buffer[2]]); }
         
      break;
    case C:
        if(buffer[1] == PRT && isPrintable(buffer[2])) { Serial.print((char)buffer[2]); }
        else if(buffer[1] == VAL) { Serial.print(buffer[2]); }
      break;
    case SA:
        if(buffer[1] == PRT && isPrintable(memory.bank_A[memory.bank_A[buffer[2]]])) { Serial.print((char)memory.bank_A[memory.bank_A[buffer[2]]]); }
        else if(buffer[1] == VAL) { Serial.print(memory.bank_A[memory.bank_A[buffer[2]]]); }
      break;
    case SB:
        if(buffer[1] == PRT && isPrintable(memory.bank_B[memory.bank_B[buffer[2]]])) { Serial.print((char)memory.bank_B[memory.bank_B[buffer[2]]]);  }
        else if(buffer[1] == VAL) { Serial.print(memory.bank_B[memory.bank_B[buffer[2]]]); }
      break;
    }
  }

void SYS_input(uint8_t buffer[4], Memory &memory){
  while(!Serial.available()){} // Espera a que el teclado este disponible
