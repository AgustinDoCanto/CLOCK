...
void ClockInterpreter::run(File &file, DisplayManager &displayManager){
    ...
    while (file.available()) {
        ...
        switch(buffer[0]){
            ...
            case SYS:
                handle_SYS_function(buffer, this->memory, displayManager, opened_file);
              break;
            case JNZ:
...
void handle_SYS_function(uint8_t buffer[4], Memory &memory, DisplayManager &console, File &file) {
  switch(buffer[1]){
    case PRT:
...
void SYS_print(uint8_t buffer[4], Memory &memory, DisplayManager &console) {
  ...
  switch(buffer[3]){ // 0: SYS 1: PRT/VAL 2: <value> 3: <bank>
    case A:
        if(buffer[1] != VAL && (char)memory.bank_A[buffer[2]] == DELETE_CODE){
      ...
      break;
    case B:
        if(buffer[1] != VAL && (char)memory.bank_B[buffer[2]] == DELETE_CODE){
          console.delOneOnBuffer();
          return;
        }
      ...
      break;
    case C:
        if(buffer[1] != VAL && (char)buffer[2] == DELETE_CODE){
          console.delOneOnBuffer();
          return;
        }
      ...
      break;
    case SA:
        if(buffer[1] != VAL &&(char)memory.bank_A[memory.bank_A[buffer[2]]] == DELETE_CODE){
          console.delOneOnBuffer();
          return;
        }
      ...
      break;
    case SB:
        if(buffer[1] != VAL && (char)memory.bank_B[memory.bank_B[buffer[2]]] == DELETE_CODE){
          console.delOneOnBuffer();
          return;
        }
