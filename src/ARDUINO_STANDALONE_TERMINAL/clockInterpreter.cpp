...
void ClockInterpreter::run(File &file, DisplayManager &displayManager){
    SD.begin(SD_CS);
    ...
    while (file.available()) {
        ...
        switch(buffer[0]){
            ...
            case DIV:
              ...
              break;
            case SYS:
                handle_SYS_function(buffer, this->memory, displayManager);
              break;
            case JNZ:
...
void Memory::set_position_value(uint8_t register_map, uint8_t pos, int value){
   switch(register_map){
    case A: this->bank_A[pos] = (uint8_t)value; break;
    case B: this->bank_B[pos] = value; break;
   }
}
  
int Memory::get_position_value(uint8_t register_map, uint8_t pos) {
    switch(register_map){
      case A: return (int)this->bank_A[pos]; break;
      case B: return this->bank_B[pos]; break;
    }
  }
...
void SYS_print(uint8_t buffer[4], Memory &memory, DisplayManager &console) {
  switch(buffer[3]){ // SYS PRT <value> <bank>
    case A:
      if((char)memory.bank_A[buffer[2]] == 8){ // Caracter-Codigo designado para borrar
      ...
      break;
    case B:
      if((char)memory.bank_B[buffer[2]] == 8){ // Caracter-Codigo designado para borrar
        console.delOneOnBuffer();
        console.updateText();
        return;
      }
      ...
      break;
    case C:
      if((char)buffer[2] == 8){ // Caracter-Codigo designado para borrar
        console.delOneOnBuffer();
        console.updateText();
        return;
      }
...
void SYS_input(uint8_t buffer[4], Memory &memory){
  while(Serial.available() == 0){} // Espera a que el teclado este disponible
